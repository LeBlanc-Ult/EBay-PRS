...
bias.corr.kernel <- function(zz, xout, bw = "nrd0")
{
  ...
  truez = D1ss(x = zz, y = Psi.z, xout = xout, spl.spar = NULL)
  return(truez)
}
...
## example
#zval = rnorm(1000)
#MAF.sim = runif(n = 1000, min = 0.05, max = 0.5)
#obj = EBay.PRS.binary(zval, preval = 0.1, caseNo = 10000, ctrlNo = 10000, 
#                      MAF = MAF.sim, nulltype = 0, df = 7, bre = 120)


ieu <- read.delim("/Users/Lydia/UCSD/SP21/CSE284/Proj/GWAS_ES", header = FALSE)
names(ieu) <- c("#CHROM", "POS", "ID", "REF", "ALT", "AF", "BETA", "SE")
ieu$ZVAL <- ieu$BETA / ieu$SE
ieu$MAF <- 1 - ieu$AF

gwas <- read.delim("/Users/Lydia/UCSD/SP21/CSE284/Proj/rs_gwas.txt", header = FALSE)
names(gwas) <- c("index","#CHROM", "POS", "ID", "REF", "ALT", "AF", "BETA", "SE")
gwas$ZVAL <- gwas$BETA / gwas$SE
gwas$MAF <- 1 - gwas$AF
gwas_1 <- unique(rbind(gwas[,-1],ieu))

zval <- qnorm(pt(gwas_1$ZVAL,df=7))
MAF.sim <- apply(gwas_1[,c("MAF", "AF")], 1, min)
obj <- EBay.PRS.binary(zval, preval = 0.117, caseNo = 122977, ctrlNo = 105974, 
                       MAF = MAF.sim, nulltype = 0, df = 7, bre = 120)
gwas_1$corrbeta <- obj$beta.corr.Tweedie
all <- merge(ieu,gwas_1,by.x = "ID", by.y = "ID", all.x = T, all.y = F)
all=all[,c(1:8,20)]
names(all) <- c("iID","#CHROM", "POS",  "REF", "ALT", "AF", "BETA", "SE","corrBETA")
write.table(all,file = "/Users/Lydia/UCSD/SP21/CSE284/Proj/corrected_beta.txt", sep = "\t", col.names = T, row.names = F, quote =FALSE)

write.table(all[,1], file = "/Users/Lydia/UCSD/SP21/CSE284/Proj/rsIDs_new.txt", sep = "\n", col.names = F, row.names = F, quote =FALSE)
